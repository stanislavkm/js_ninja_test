...
export default {
  ...
  methods:{
    add(){
      ...
      setInterval(async()=>{
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${currentTicker.name}&tsyms=USD&api_key=eddc13aa0115c4cfbf4970c36efa17169041a5dad46233b3b8825f9a375d5881`
        );
        const data = await f.json()
        console.log(data)
        this.tickers.find(i=>i.name===currentTicker.name).price = data.USD > 1 ? data.USD.toFixed(2) : data.USD.toPrecision(2)
